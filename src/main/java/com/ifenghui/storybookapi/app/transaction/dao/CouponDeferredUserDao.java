 ...
 */
@Transactional
public interface CouponDeferredUserDao extends JpaRepository<CouponDeferredUser, Long> ,JpaSpecificationExecutor {
    ...
    @Query("from CouponDeferredUser c where c.userId =:userId  and c.endTime<:nowTime")
    Page<CouponDeferredUser> getExpiredUserDeferredCoupons(@Param("userId") Long userId, @Param("nowTime") Date nowTime, Pageable pageable);//已过期的
    ...
    @Query("select count(c) from CouponDeferredUser c where c.userId =:userId  and c.endTime>=:nowTime")
    Integer getUserDeferredCouponsCount(@Param("userId") Long userId, @Param("nowTime") Date nowTime);//未过期的
